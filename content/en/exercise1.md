...
| -------- | ------------ | ---------------------------- | --------------- |
| `default` | Fedora VM **created inside OpenShift** (KubeVirt), `fedora-mol-[[ocpUser]]` | `create.yml` provisions the VM (cloud-init and SSH key); `destroy.yml` removes it at the end. | **Only from Dev Spaces**, with the OpenShift API URL, user and password from the header form and namespace `virtualization-test-[[ocpUser]]` (see 5.3.1). |
...
This scenario runs **only from Dev Spaces**. It creates a new Fedora VM with KubeVirt, injects the lab public key through cloud-init, waits for the VMI IP, and applies the playbook over SSH. The YAML in the steps below is what makes the VM reachable. The OpenShift API URL, user and password come from the header form.
...
The first step of `molecule test` is `destroy`, so `molecule_vars.yml` must be filled in before any command of this scenario. That file holds the OpenShift username and password: **do not** push it to Gitea or any public remote. Exclude it from `MANIFEST.in`.

Cloud-init installs `ssh_tests_connections/id_fedora_new.pub` (it ships in the repository). The SSH user of this VM is `fedora`, the user of the `quay.io/containerdisks/fedora` image. The platform is named `fedora-mol-[[ocpUser]]` and lives in `virtualization-test-[[ocpUser]]`, the same project as the inventory Fedora (`[[fedoraAlias]]`). `destroy` removes only `fedora-mol-[[ocpUser]]`.

##### Step 1 — `molecule/default/molecule.yml`

Define the `default` driver (Molecule 26.x in the lab image), the platform, the `servers` group inventory, and the test sequence including `prepare` (lint). `ansible_host` reads the IP that `create.yml` writes.
...
**Result in the project:** Molecule recognizes the `default` scenario; the play `hosts: servers` resolves to `fedora-mol-[[ocpUser]]`. `roles_path` and `collections_path` let `converge` find the project roles and the KubeVirt collections.

##### Step 2 — `molecule/default/molecule_vars.yml`

OpenShift credentials for `create.yml` and `destroy.yml`. The API URL, user and password come from the header form. **Do not** push real secrets to a public remote.
...
The namespace follows the form `virtualization-test-[[ocpUser]]` (the project where your user can create VirtualMachines). **Do not** push this file to a public remote.

##### Collections — `molecule/default/requirements.yml`

Molecule installs this file in the `dependency` step (Galaxy), before `create`. It is required for `kubevirt.core.kubevirt_vm` and for deleting the Service in `destroy.yml`.
...
**Result in the project:** `molecule test` installs the three collections under `~/.ansible/collections` (the path set in `molecule.yml`).

##### Step 3 — `molecule/default/create.yml`

Logs in to OpenShift, creates the VM with a Fedora disk and cloud-init (SSH key), waits until the VMI has an IP, saves it for `converge`, and waits for port 22.
...
`create.yml` reads the IP with `oc get vmi` and writes it to `/tmp/molecule-default-[[ocpUser]].ip`. That path must match the `ansible_host` lookup in `molecule.yml`. Memory is **4Gi**: with less, WildFly may never listen on port 8080. `no_log` keeps the `oc login` password out of the output.

**Result on OpenShift:** VM `fedora-mol-[[ocpUser]]` running in `virtualization-test-[[ocpUser]]`, with `id_fedora_new.pub` installed for user `fedora` and SSH on port 22.

##### Step 4 — `molecule/default/destroy.yml`

Logs in again (Molecule runs `destroy` as a separate playbook) and removes the VM and the associated Service.
...
**Result on OpenShift:** the test VM and the Service `svc-fedora-mol-[[ocpUser]]`, if it existed, are gone. `ignore_errors` lets the scenario continue if the VM was never created.
...
**Result on the node:** `wildfly` active, port 8080 listening, and HTTP 200 on `/sample/`. The wait is 120 s with up to 8 retries: on a freshly created VM, WildFly takes longer to publish `/sample/`.

**Final result of this block (5.3.1 — `default` scenario):** `molecule/default/` tree with `molecule.yml`, `molecule_vars.yml`, `requirements.yml`, `create.yml`, `destroy.yml`, `prepare.yml`, `converge.yml`, and `verify.yml`.
...
- `molecule/with_existing_machine/requirements.yml` — at least `community.general` (Molecule Galaxy installs it in `dependency`). The KubeVirt collections belong in `molecule/default/requirements.yml` (section 5.3.1), not at the project root.
...
- `default` runs **only from Dev Spaces** with the header-form values: `[[ocpApiUrl]]`, user `[[ocpUser]]`, and namespace `virtualization-test-[[ocpUser]]` (`molecule test` without `-s` uses `default`).

Default scenario (`default`, creates and destroys an OpenShift VM; **Dev Spaces only**). `molecule_vars.yml` contains `[[ocpApiUrl]]`, user `[[ocpUser]]`, the header-form password, and namespace `virtualization-test-[[ocpUser]]`.
...
After `create`, VM `fedora-mol-[[ocpUser]]` must exist in `virtualization-test-[[ocpUser]]`. After `destroy`, that VM must be gone. The inventory Fedora stays in the same project.
...
**Final result of this block (5.3 — Molecule):** both scenarios are defined on disk. `molecule test -s with_existing_machine` completes `destroy` → `create` → `prepare` (lint) → `converge` → `verify` → `destroy` with exit code `0` on the classroom Fedora and leaves it running. `molecule test -s default` reaches exit code `0` with API URL `[[ocpApiUrl]]`, user `[[ocpUser]]`, and namespace `virtualization-test-[[ocpUser]]`: it creates `fedora-mol-[[ocpUser]]`, deploys WildFly, and deletes that VM at the end.
...
5. Validate with yamllint, ansible-lint (playbook + roles + `group_vars`), and Molecule (section 5). Complete **`molecule test -s with_existing_machine`** against your Fedora. The `default` scenario creates another Fedora in `virtualization-test-[[ocpUser]]` with API URL `[[ocpApiUrl]]`, user `[[ocpUser]]`, and the password from the header form, and you run it with **`molecule test -s default`**.
