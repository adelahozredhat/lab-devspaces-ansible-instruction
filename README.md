...
Las capturas de esta sección son de un laboratorio de ejemplo. Verás un clúster concreto, el usuario OpenShift `user2` y el usuario Gitea `lab-user-2`. **Las URLs, las rutas y los usuarios de acceso dependen de la cuenta que te hayan asignado.** Usa los datos del Excel del laboratorio (o las cajas *Datos del laboratorio* de la [guía en GitHub Pages](index.html)). No copies las rutas de las imágenes tal cual.
...
Cada ejercicio vive en **su propio repositorio Git**. El arranque habitual del laboratorio es **un único workspace** creado desde el repositorio de instrucciones (apartado anterior): el `devfile.yaml` clona todos los proyectos. Trabaja en la carpeta del ejercicio que toque y abre su `README.md`. También puedes crear un workspace solo con el repo de un ejercicio si el formador lo indica.

Flujo habitual:

1. Accede a Dev Spaces y **arranca el workspace** con la URL de **tu** repo de instrucciones (sección anterior).
